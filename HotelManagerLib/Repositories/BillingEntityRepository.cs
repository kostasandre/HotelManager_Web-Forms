...
namespace HotelManagerLib.Repositories
{
    ...
    public class BillingEntityRepository : IEntityRepository<Billing>
    {
        ...
        public IList<Billing> ReadAllList()
        {
            using (var context = new DataBaseContext())
            {
                return context.Billings.Include("BillingServices").Include("Booking").Include("Booking.Customer").ToList();
            }
        }
        ...
        public Billing ReadOne(int id)
        {
            using (var context = new DataBaseContext())
            {
                var billing = context.Billings.Include("BillingServices").SingleOrDefault(x => x.Id == id);
                return billing;
            }
        }
